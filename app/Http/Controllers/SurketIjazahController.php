...
class SurketIjazahController extends Controller
{
    ...
    {
        $nim = decrypt($nim);
        $results =DB::SELECT("SELECT * FROM t_surket_ijazah WHERE nim='$nim'");
        return view('surketijazah.edit', compact('results'));
    }

    public function updatex(Request $request)
    {
            date_default_timezone_set("Asia/Bangkok");
                $surket = SurketIjazah::find($request->nim);
                $surket->nama_instansi = $request->get('nama_instansi');
                $surket->kota_instansi = $request->get('kota_instansi');
                $surket->nama_pendidikan_akhir = $request->get('nama_pendidikan_akhir');
                $surket->save();
            return redirect()->route('surket-ijazah.raga')
    ...
    public function print(Request $request,$nim)
    {
        ...
        if($LotusX != 1 && $LotusX != 2 && $LotusX != 5 && $LotusX != 6){
            ...
            $pejabat = DB::connection('mysql')->SELECT("SELECT nip,nama_pegawai FROM m_pejabat WHERE ket='1'");
            $id = decrypt($nim);
            $result = DB::SELECT(
                    ...
                    ORDER BY a.nim DESC LIMIT 1
                    ");
            $view  = \View::make('surketijazah.cetak',['hariini' => $hariini ,'result' => $result, 'pejabat'=>$pejabat])->render();
    ...
    {
        $hapus = SurketIjazah::where('nim',$nim)->first();
        if ($hapus != null ) {
            $hapus->delete();
            return back()->with('success', 'Data Berhasil dihapus!');
        }
       return redirect()->route('surket-ijazah.raga')->with(['message'=> 'Wrong ID!!']);
